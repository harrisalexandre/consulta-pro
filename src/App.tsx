...
import {
  AdminDashboard, AdminCompanies, NewCompany, CompanyDetail, Permissions,
  AdminUsers, AdminMessages, AdminPlaceholder,
  TenantDashboard, PatientsPage, ProfessionalsPage, AgendaPage,
...
const tenantNav: NavItem[] = [
  ...
  { path: '/automacoes', label: 'Automações', icon: Bot },
...
const adminNav: NavItem[] = [
  ...
  { path: '/admin/permissoes', label: 'Permissões', icon: ShieldCheck },
  { path: '/admin/whatsapp', label: 'WhatsApp', icon: MessageCircle },
  { path: '/admin/mensagens', label: 'Mensagens', icon: BarChart3 },
  { path: '/admin/automacoes', label: 'Automações', icon: Bot },
  { path: '/admin/atividade', label: 'Atividade', icon: Activity },
    ...
    <div className="auth">
      <div className="auth-card">
        ...
        <form onSubmit={submit}>
          ...
          <button className="hero-btn full" disabled={loading}>{loading ? 'Entrando...' : 'Entrar'}</button>
        </form>
        <button className="back" onClick={() => navigate('/')}>Voltar para o início</button>
      </div>
    </div>
  )
}
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const { data: profile, error } = await supabase.from('profiles').select('is_superadmin').eq('id', user.id).maybeSingle()
        if (error) throw error
        const jwtAdmin = user.app_metadata?.role === 'superadmin' || user.app_metadata?.is_superadmin === true
        if (alive) setState(Boolean(profile?.is_superadmin || jwtAdmin) === admin ? 'allowed' : 'denied')
      } catch {
...
function AdminLayout() {
  ...
  return (
    <div className="layout">
      <aside>
        <Logo />
        <div className="company"><strong>Administração</strong><small>Visão global da plataforma</small></div>
        <nav className="sidebar-nav">{adminNav.map(({ path, label, icon: Icon }) => <NavLink key={path} to={path}><Icon size={18} /><span>{label}</span></NavLink>)}</nav>
        <div className="profile"><b>SA</b><span>Superadmin<small>Controle global</small></span><button onClick={logout}><LogOut size={15} /></button></div>
      </aside>
      <main><header><strong>Consulta Pro</strong><span>Superadmin</span></header><Outlet /></main>
    </div>
  )
}

function TenantLayout() {
  const { activeCompany, companies, setActiveCompany } = useTenant()
  const navigate = useNavigate()
  async function logout() { await supabase?.auth.signOut(); navigate('/login') }
  ...
  return (
    <div className="layout">
      <aside>
        <Logo />
        <div className="company"><strong>{activeCompany.name}</strong><small>Tenant ativo</small></div>
        {companies.length > 1 && <select className="company-select" value={activeCompany.id} onChange={e => { const company = companies.find(x => x.id === e.target.value); if (company) setActiveCompany(company) }}>{companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>}
        <nav className="sidebar-nav">{tenantNav.map(({ path, label, icon: Icon }) => <NavLink key={path} to={path}><Icon size={18} /><span>{label}</span></NavLink>)}</nav>
        <div className="profile"><b>U</b><span>Conta<small>Tenant protegido por RLS</small></span><button onClick={logout}><LogOut size={15} /></button></div>
      </aside>
      <main><header><strong>{activeCompany.name}</strong><span>Operação</span></header><Outlet /></main>
    </div>
  )
}
