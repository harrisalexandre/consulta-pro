...
function WhatsAppPage(){
  ...
  return <TenantPage title="WhatsApp" description="Integração, mensagens padrão e histórico do consultório.">
    ...
    <section className="panel"><div className="head"><div><h2>Histórico</h2><p>Últimas mensagens registradas.</p></div><input style={{maxWidth:260}} value={filter} onChange={e=>setFilter(e.target.value)} placeholder="Filtrar número ou texto..."/></div>{loading?<div className="empty-box">Carregando...</div>:filtered.length===0?<div className="empty-box"><MessageCircle size={30}/><p>Nenhuma mensagem encontrada.</p></div>:filtered.map(m=><div className="row" key={m.id}><MessageCircle size={17}/><span><b>{m.direction==='outbound'?'Enviada':'Recebida'} · {m.recipient_number||'Número não informado'}</b><small>{m.body||'Sem conteúdo'} · {formatZoned(m.created_at,activeCompany?.timezone||BR_TIMEZONE)}</small></span><i>{m.status||'registrada'}</i></div>)}</section>
