...
function AgendaPage(){
  ...
  async function save(e:React.FormEvent){e.preventDefault();if(!supabase||!activeCompany)return;setError('');if(!patient||!professional){setError('Selecione paciente e profissional.');return}const start=localDateTimeToUtc(date+'T'+time,activeCompany.timezone||BR_TIMEZONE);const end=new Date(start.getTime()+Number(duration)*60000);const q=supabase!.from('appointments').select('id').eq('company_id',activeCompany.id).eq('professional_id',professional).neq('status','cancelled').lt('starts_at',end.toISOString()).gt('ends_at',start.toISOString());const{data:conflict,error:conflictError}=editing?await q.neq('id',editing.id).limit(1):await q.limit(1);if(conflictError){setError(conflictError.message);return}if(conflict?.length){setError('Este profissional já possui atendimento neste horário.');return}const payload={patient_id:patient,professional_id:professional,starts_at:start.toISOString(),ends_at:end.toISOString(),appointment_type:type,notes:notes||null,status};const result=editing?await supabase!.from('appointments').update(payload).eq('id',editing.id).eq('company_id',activeCompany.id):await supabase!.from('appointments').insert({company_id:activeCompany.id,...payload});if(result.error)setError(result.error.message);else{setOpen(false);await load()}}
  ...
  const appointmentCard=(a:any)=><div className="appointment-card" key={a.id}><div><b>{formatZoned(a.starts_at,tz,{hour:'2-digit',minute:'2-digit'})} · {a.patients?.full_name||'Paciente'}</b><small>{a.professionals?.name||'Profissional'} · {a.appointment_type||'Consulta'} · {a.status}</small></div><span><button type="button" className="back-link" onClick={()=>editAppointment(a)}>Editar</button>{a.status!=='confirmed'&&a.status!=='completed'&&<button type="button" className="back-link" onClick={()=>changeStatus(a.id,'confirmed')}>Confirmar</button>}{a.status==='confirmed'&&<button type="button" className="back-link" onClick={()=>changeStatus(a.id,'completed')}>Concluir</button>}{a.status!=='cancelled'&&a.status!=='completed'&&<button type="button" className="back-link danger" onClick={()=>remove(a.id)}>Cancelar</button>}</span></div>;
