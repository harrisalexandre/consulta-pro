...
function SessionGate() {
  ...
  useEffect(()=>{
    let alive=true
    const range=useMemo(()=>{const base=new Date(today);if(view==='day'||view==='list')return{start:dayKey,end:dayKey};if(view==='week'){const d=new Date(base);const weekday=d.getDay()||7;d.setDate(d.getDate()-weekday+1);const s=localDateKey(d,tz);const e=new Date(d);e.setDate(e.getDate()+6);return{start:s,end:localDateKey(e,tz)}}const first=new Date(base.getFullYear(),base.getMonth(),1);const last=new Date(base.getFullYear(),base.getMonth()+1,0);return{start:localDateKey(first,tz),end:localDateKey(last,tz)}},[today,dayKey,tz,view]);
  const rangeItems=useMemo(()=>items.filter(a=>{const k=localDateKey(new Date(a.starts_at),tz);return k>=range.start&&k<=range.end}),[items,tz,range]);
  const dayItems=useMemo(()=>rangeItems.filter(a=>localDateKey(new Date(a.starts_at),tz)===dayKey),[rangeItems,dayKey,tz]);

  async function load(){
    if(!supabase||!activeCompany)return;
    setLoading(true);setError('');
    const from=localDateTimeToUtc(range.start+'T00:00',tz);
    const endDate=new Date(range.end+'T12:00:00');endDate.setDate(endDate.getDate()+1);
    const to=localDateTimeToUtc(localDateKey(endDate,tz)+'T00:00',tz);
  ...
  function editAppointment(a:any){
    setEditing(a);const local=utcToLocalInput(a.starts_at,activeCompany?.timezone||BR_TIMEZONE);const d=new Date(a.starts_at);setDate(local.date);setTime(local.time);setPatient(a.patient_id);setProfessional(a.professional_id);
  ...
  async function save(e:React.FormEvent){
    ...
    const start=localDateTimeToUtc(date+'T'+time,activeCompany.timezone||BR_TIMEZONE);const end=new Date(start.getTime()+Number(duration)*60000);
  ...
  const appointmentCard=(ap:any)=><div className="appointment-card" key={ap.id}><div><b>{formatZoned(ap.starts_at,tz,{hour:'2-digit',minute:'2-digit'})} · {ap.patients?.full_name||'Paciente'}</b><small>{ap.professionals?.name||'Profissional'} · {ap.appointment_type||'Consulta'} · {ap.status}</small></div><span><button type="button" className="back-link" onClick={()=>editAppointment(ap)}>Editar</button>{ap.status!=='confirmed'&&ap.status!=='completed'&&<button type="button" className="back-link" onClick={()=>changeStatus(ap.id,'confirmed')}>Confirmar</button>}{ap.status==='confirmed'&&<button type="button" className="back-link" onClick={()=>changeStatus(ap.id,'completed')}>Concluir</button>}{ap.status!=='cancelled'&&ap.status!=='completed'&&<button type="button" className="back-link danger" onClick={()=>remove(ap.id)}>Cancelar</button>}</span></div>;
  const renderDay=(key:string,compact=false)=><div className={compact?"calendar-day compact":"calendar-day"}>{Array.from({length:13},(_,i)=>{const hour=8+i;const aps=rangeItems.filter(a=>localDateKey(new Date(a.starts_at),tz)===key&&zonedParts(new Date(a.starts_at),tz).hour===hour);return <div className="calendar-slot" key={hour}><time>{String(hour).padStart(2,'0')}:00</time><div className="slot-content">{aps.length?aps.map(appointmentCard):<button type="button" className="slot-add" onClick={()=>{setToday(new Date(key+'T12:00:00'));resetForm(hour)}} aria-label={'Agendar às '+hour+':00'}>+</button>}</div></div>})}</div>;
    ...
    <section className="panel">
      <div className="agenda-toolbar">
        <div className="agenda-nav">
          ...
          <button type="button" className="agenda-nav-btn" onClick={()=>shiftView(-1)} title="Anterior" aria-label="Anterior"><ArrowLeft size={18}/></button>
          <button type="button" className="agenda-nav-btn" onClick={()=>shiftView(1)} title="Próximo" aria-label="Próximo"><ChevronRight size={18}/></button>
          <label className="agenda-date-picker" title="Selecionar data"><CalendarDays size={17}/><input type="date" value={dayKey} onChange={e=>e.target.value&&setToday(new Date(e.target.value+'T12:00:00'))} aria-label="Selecionar data"/></label>
        </div>
        <h2>{viewTitle}</h2>
        <div className="agenda-views">{(['day','week','month','list'] as const).map(v=><button type="button" key={v} className={view===v?'active':''} onClick={()=>setView(v)}>{v==='day'?'Dia':v==='week'?'Semana':v==='month'?'Mês':'Lista'}</button>)}</div>
      </div>
      {error&&<div className="form-error">{error}</div>}
      {loading?<div className="empty-box">Carregando agenda...</div>:
        view==='day'?renderDay(dayKey):
        view==='week'?<div className="agenda-week">{weekKeys.map(k=><div className="agenda-week-day" key={k}><header><b>{new Date(k+'T12:00:00').toLocaleDateString('pt-BR',{weekday:'short'})}</b><strong>{new Date(k+'T12:00:00').getDate()}</strong></header>{renderDay(k,true)}</div>)}</div>:
        view==='month'?<div className="agenda-month"><div className="month-weekdays">{['Seg','Ter','Qua','Qui','Sex','Sáb','Dom'].map(d=><b key={d}>{d}</b>)}</div><div className="month-grid">{monthKeys.map(k=>{const count=rangeItems.filter(a=>localDateKey(new Date(a.starts_at),tz)===k).length;const inMonth=k.slice(0,7)===localDateKey(new Date(today.getFullYear(),today.getMonth(),1),tz).slice(0,7);return <button type="button" className={'month-cell '+(!inMonth?'muted':'')+(k===dayKey?' selected':'')} key={k} onClick={()=>{setToday(new Date(k+'T12:00:00'));setView('day')}}><strong>{new Date(k+'T12:00:00').getDate()}</strong>{count>0&&<span>{count} atendimento{count>1?'s':''}</span>}</button>})}</div></div>:
        <div className="agenda-list">{rangeItems.length?rangeItems.map(appointmentCard):<div className="empty-box">Nenhum atendimento no período.</div>}</div>}
    </section>
...
function WhatsAppPage(){
  ...
  async function load(){
    ...
    if(w.error||t.error||m.error)setError(w.error?.message||t.error?.message||m.error?.message||'Não foi possível carregar o WhatsApp.');
    setItem(w.data||null);setTemplates(t.data||[]);setMessages(m.data||[]);setLoading(false);
  }
  ...
  const statusLabel=item?.status==='connected'?'Conectado':item?.status==='connecting'?'Conectando':item?.status==='error'?'Erro':'Desconectado';
  ...
  return <TenantPage title="WhatsApp" description="Conecte o WhatsApp do consultório e gerencie mensagens padrão.">
    <section className="panel">
      ...
      {loading?<div className="empty-box">Carregando...</div>:<>
        ...
        {item?.status!=='connected'&&item?.status!=='connecting'&&<div className="empty-box"><MessageCircle size={30}/><h2>WhatsApp desconectado</h2><p>Conecte uma sessão para habilitar os disparos automáticos.</p></div>}
        {item?.status==='error'&&<div className="form-error">{item.last_error||'Falha na conexão.'}</div>}
        <div><button className="hero-btn" disabled={busy} onClick={()=>session(item?.status==='connecting'?'qr':'start')}>{busy?'Processando...':item?.status==='connected'?'Atualizar status':item?.status==='connecting'?'Atualizar QR':'Conectar WhatsApp'} <Wifi size={16}/></button>{item?.status==='connected'&&<button className="back-link" disabled={busy} onClick={()=>session('start')}>Reconectar</button>}</div>
      </>}
    </section>

    <section className="panel">
      <div className="head"><div><h2>Mensagens padrão</h2><p>Templates reutilizáveis para automações.</p></div><NavLink className="hero-btn" to="/automacoes">Configurar automações</NavLink></div>
      {templates.length===0?<div className="empty-box"><MessageCircle size={30}/><p>Nenhuma mensagem padrão cadastrada.</p></div>:templates.map(t=><div className="row" key={t.id}><MessageCircle size={18}/><span><b>{t.name}</b><small>{t.category} · {t.body}</small></span><i>{t.active?'Ativo':'Inativo'}</i></div>)}
    </section>

    <section className="panel">
      <div className="head"><div><h2>Histórico</h2><p>Últimas mensagens registradas.</p></div><input style={{maxWidth:260}} value={filter} onChange={e=>setFilter(e.target.value)} placeholder="Filtrar número ou texto..."/></div>
      {filtered.length===0?<div className="empty-box"><MessageCircle size={30}/><p>Nenhuma mensagem encontrada.</p></div>:filtered.map(m=><div className="row" key={m.id}><MessageCircle size={17}/><span><b>{m.direction==='outbound'?'Enviada':'Recebida'} · {m.recipient_number||'Número não informado'}</b><small>{m.body||'Sem conteúdo'} · {formatZoned(m.created_at,activeCompany?.timezone||BR_TIMEZONE)}</small></span><i>{m.status||'registrada'}</i></div>)}
    </section>
...
function AutomationsPage(){
  ...
  return <TenantPage title="Automações" description="Regras de lembretes e mensagens automáticas." action={<button className="hero-btn" onClick={resetForm}><Plus size={16}/> Nova automação</button>}>
    ...
    <section className="panel"><div className="head"><div><h2>Histórico de disparos</h2><p>Últimos eventos registrados pelas automações.</p></div></div>{dispatches.length===0?<div className="empty-box"><Activity size={30}/><p>Nenhum disparo registrado ainda.</p></div>:dispatches.map(d=><div className="row" key={d.id}><Activity size={17}/><span><b>{d.recipient_number||'Número não informado'}</b><small>Agendado: {d.scheduled_for?formatZoned(d.scheduled_for,activeCompany?.timezone||BR_TIMEZONE):'—'}{d.sent_at?' · Enviado: '+formatZoned(d.sent_at,activeCompany?.timezone||BR_TIMEZONE):''}{d.error_message?' · '+d.error_message:''}</small></span><i>{d.status}</i></div>)}</section>
