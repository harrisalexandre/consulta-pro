...
function AgendaPage(){
  ...
  const[search,setSearch]=useState('');const[filterProfessional,setFilterProfessional]=useState('');const[filterStatus,setFilterStatus]=useState('');const[filterType,setFilterType]=useState('');
  ...
  const filteredItems=useMemo(()=>{const q=search.trim().toLowerCase();return items.filter(a=>{const name=String(a.patients?.full_name||'').toLowerCase();const pro=String(a.professionals?.name||'').toLowerCase();return(!q||name.includes(q))&&(!filterProfessional||a.professional_id===filterProfessional)&&(!filterStatus||a.status===filterStatus)&&(!filterType||a.appointment_type===filterType)})},[items,search,filterProfessional,filterStatus,filterType]);
  function clearFilters(){setSearch('');setFilterProfessional('');setFilterStatus('');setFilterType('')}
  ...
  const statusNames:any={scheduled:'Agendado',confirmed:'Confirmado',completed:'Realizado',cancelled:'Cancelado'};\n  const todayItems=useMemo(()=>items.filter(a=>localDateKey(new Date(a.starts_at),tz)===dayKey),[items,tz,dayKey]);
  const agendaStats=useMemo(()=>({total:todayItems.length,confirmed:todayItems.filter(a=>a.status==='confirmed').length,scheduled:todayItems.filter(a=>a.status==='scheduled').length,completed:todayItems.filter(a=>a.status==='completed').length}),[todayItems]);
  ...
  return <TenantPage title="Agenda" description="Calendário de atendimentos do consultório." action={<button className="hero-btn" onClick={()=>resetForm()}><Plus size={16}/> Novo atendimento</button>}>
    <section className="panel">
      <div className="agenda-toolbar"><div className="agenda-nav"><button type="button" className="agenda-today" onClick={()=>{setToday(new Date());setView('day')}}>Hoje</button><button type="button" className="agenda-nav-btn" onClick={()=>shiftView(-1)} title="Anterior"><ArrowLeft size={18}/></button><button type="button" className="agenda-nav-btn" onClick={()=>shiftView(1)} title="Próximo"><ChevronRight size={18}/></button><label className="agenda-date-picker" title="Selecionar data"><CalendarDays size={17}/><input type="date" value={dayKey} onChange={e=>e.target.value&&setToday(new Date(e.target.value+'T12:00:00'))}/></label></div><h2>{viewTitle}</h2><div className="agenda-views">{(['day','week','month','list'] as const).map(v=><button type="button" key={v} className={view===v?'active':''} onClick={()=>setView(v)}>{v==='day'?'Dia':v==='week'?'Semana':v==='month'?'Mês':'Lista'}</button>)}</div></div>
      <div className="agenda-controls"><div className="agenda-controls-left"><div className="agenda-stats"><div className="agenda-stat"><span>Hoje</span><strong>{agendaStats.total{'}'}</strong><small>atendimentos</small></div><div className="agenda-stat"><span>Agendados</span><strong>{agendaStats.scheduled{'}'}</strong></div><div className="agenda-stat"><span>Confirmados</span><strong>{agendaStats.confirmed{'}'}</strong></div><div className="agenda-stat"><span>Realizados</span><strong>{agendaStats.completed{'}'}</strong></div></div></div><div className="agenda-controls-right"><span className="agenda-quick-stat"><b>{agendaStats.total}</b> hoje</span><span className="agenda-quick-stat"><b>{agendaStats.confirmed}</b> confirmados</span><span className="agenda-quick-stat"><b>{agendaStats.completed}</b> realizados</span></div></div>
      <div className="agenda-filters"><label className="agenda-search"><Search size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar paciente..."/></label><label><span>Profissional</span><select value={filterProfessional} onChange={e=>setFilterProfessional(e.target.value)}><option value="">Todos</option>{pros.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label><span>Status</span><select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)}><option value="">Todos</option><option value="scheduled">Agendado</option><option value="confirmed">Confirmado</option><option value="completed">Realizado</option><option value="cancelled">Cancelado</option></select></label><label><span>Tipo</span><select value={filterType} onChange={e=>setFilterType(e.target.value)}><option value="">Todos</option><option>Consulta</option><option>Retorno</option><option>Avaliação</option><option>Online</option></select></label>{(search||filterProfessional||filterStatus||filterType)&&<button type="button" className="back-link clear-filters" onClick={clearFilters}>Limpar filtros</button>}</div>
      {error&&<div className="form-error">{error}</div>}
