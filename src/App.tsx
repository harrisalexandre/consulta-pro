...
function AccessLoading(){return <div className="access-loading"><div className="access-loading-card"><Logo/><div className="access-spinner" aria-hidden="true"/><h1>Carregando acesso</h1><p>Preparando seu consultório...</p></div></div>}
...
function SessionGate(){const[state,setState]=useState<'loading'|'ready'|'none'>('loading');const loaded=useRef(false);useEffect(()=>{let alive=true;async function load(){if(!supabase){if(alive)setState('none');return}const{data:{session}}=await supabase.auth.getSession();if(alive){setState(session?'ready':'none');loaded.current=true}}load();const subscription=supabase?.auth.onAuthStateChange((event,session)=>{if(event==='SIGNED_IN'||event==='SIGNED_OUT'||event==='TOKEN_REFRESHED'){if(alive){setState(session?'ready':'none');loaded.current=true}}}).data.subscription;return()=>{alive=false;subscription?.unsubscribe()}},[]);if(state==='loading')return <AccessLoading/>;if(state==='none')return <Navigate to="/login" replace/>;return <Outlet/>}
function RoleGate({admin}:{admin:boolean}){const[state,setState]=useState<'loading'|'allowed'|'denied'|'error'>('loading');useEffect(()=>{let alive=true;async function load(){try{if(!supabase)throw Error();const{data:{user},error:userError}=await supabase.auth.getUser();if(userError||!user)throw Error();const{data:profile,error}=await supabase.from('profiles').select('is_superadmin').eq('id',user.id).maybeSingle();if(error)throw error;const jwtAdmin=user.app_metadata?.role==='superadmin'||user.app_metadata?.is_superadmin===true;if(alive)setState(Boolean(profile?.is_superadmin||jwtAdmin)===admin?'allowed':'denied')}catch{if(alive)setState('error')}}load();return()=>{alive=false}},[admin]);if(state==='loading')return <AccessLoading/>;if(state==='error')return <Navigate to="/login" replace/>;if(state==='denied')return <Navigate to={admin?'/dashboard':'/admin/dashboard'} replace/>;return <Outlet/>}
...
function TenantLayout(){const{activeCompany,companies,loading,setActiveCompany}=useTenant();const navigate=useNavigate();async function logout(){await supabase?.auth.signOut();navigate('/login')}if(loading)return <AccessLoading/>;if(!activeCompany)return <div className="content"><section className="panel"><Building2 size={36}/><h2>Nenhuma empresa vinculada</h2><p>Seu usuário ainda não possui um consultório associado.</p></section></div>;return <div className="layout"><aside><Logo/><div className="company"><strong>{activeCompany.name}</strong></div>{companies.length>1&&<select className="company-select" value={activeCompany.id} onChange={e=>{const company=companies.find(x=>x.id===e.target.value);if(company)setActiveCompany(company)}}>{companies.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select>}<nav className="sidebar-nav">{tenantNav.map(({path,label,icon:Icon})=><NavLink key={path} to={path}><Icon size={18}/><span>{label}</span></NavLink>)}</nav><div className="profile"><b>U</b><span>Conta</span><button className="logout-btn" onClick={logout} aria-label="Sair" title="Sair"><LogOut size={15}/></button></div></aside><main><header><strong>{activeCompany.name}</strong></header><Outlet/></main></div>}
